...
import 'package:alhai_ai/src/screens/ai/ai_sales_forecasting_screen.dart';
...
void main() {
  ...
  group('AiSalesForecastingScreen', () {
    testWidgets('renders without error', (tester) async {
      _setLargeViewport(tester);
      await tester.pumpWidget(
        _buildTestableWidget(const AiSalesForecastingScreen()),
      );
      await tester.pump(const Duration(seconds: 3));

      expect(find.byType(AiSalesForecastingScreen), findsOneWidget);
    });

    testWidgets('has three tabs for daily, weekly, monthly', (tester) async {
      _setLargeViewport(tester);
      await tester.pumpWidget(
        _buildTestableWidget(const AiSalesForecastingScreen()),
      );
    ...
    testWidgets('shows data after loading', (tester) async {
      ...
      // After data loads, the screen should show its content
      expect(find.byType(AiSalesForecastingScreen), findsOneWidget);
    });
  });
}
