...
import 'package:alhai_ai/src/screens/ai/ai_smart_inventory_screen.dart';
...
void main() {
  ...
  group('AiSmartInventoryScreen', () {
    testWidgets('renders without error', (tester) async {
      _setLargeViewport(tester);
      await tester.pumpWidget(
        _buildTestableWidget(const AiSmartInventoryScreen()),
      );
      await tester.pump(const Duration(seconds: 3));

      expect(find.byType(AiSmartInventoryScreen), findsOneWidget);
    });

    testWidgets('has four tabs', (tester) async {
      _setLargeViewport(tester);
      await tester.pumpWidget(
        _buildTestableWidget(const AiSmartInventoryScreen()),
      );
