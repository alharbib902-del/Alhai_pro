...
class AiAssistantService {
  ...
  Future<AssistantResponse> _handleSalesQuery(
      String storeId, String query) async {
    ...
    final yesterdaySales = await _db.salesDao.getSalesByDate(storeId, yesterday);
    final yesterdayTotal =
        yesterdaySales.fold<double>(0, (sum, s) => sum + s.total);
...
${todayTotal > yesterdayTotal ? 'أداء ممتاز! استمر على هذا المنوال.' : 'جرب تفعيل بعض العروض لزيادة المبيعات.'}''',
  ...
  Future<AssistantResponse> _handleStockQuery(String storeId) async {
    ...
    return AssistantResponse(
      ...
      data: {
        ...
        'lowStockItems':
            lowStock.take(5).map((p) => {'name': p.name, 'qty': p.stockQty}).toList(),
      },
      confidence: 0.95,
      suggestedActions: [
        const SuggestedAction(
  ...
  Future<AssistantResponse> _handleDebtQuery(String storeId) async {
    ...
    if (debtors.isNotEmpty) {
      ...
      for (final d in debtors.take(5)) {
        buffer.writeln(
            '  - ${d.name}: ${d.balance.toStringAsFixed(2)} ر.س');
      }
      if (debtors.length > 5) {
  ...
  Future<AssistantResponse> _handleTopProductsQuery(String storeId) async {
    ...
    final sales =
        await _db.salesDao.getSalesByDateRange(storeId, weekAgo, today);
    ...
    for (var i = 0; i < topProducts.length; i++) {
      ...
      buffer.writeln(
          '${i + 1}. ${p.name} - ${revenue.toStringAsFixed(2)} ر.س');
    }
