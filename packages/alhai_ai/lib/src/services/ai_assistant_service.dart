...
${todayTotal > yesterdayTotal ? 'أداء ممتاز! استمر على هذا المنوال.' : 'جرب تفعيل بعض العروض لزيادة المبيعات.'}''',
  ...
  Future<AssistantResponse> _handleDebtQuery(String storeId) async {
    // Wave 8 (P0-33): pull the headline total from SQL so it stays
    // correct even when the store has more than `getReceivableAccounts`
    // can return in one page. The bounded fetch below is fine for the
    // "top 5 debtors" list — if the store has 100s of debtors we still
    // get the largest few, which is what the chat reply shows.
    final totalDebt = await _db.accountsDao.getTotalReceivable(storeId);
  ...
  Future<AssistantResponse> _handleTopProductsQuery(String storeId) async {
    ...
    // Wave 8 (P0-33): we only need to know whether any sales exist in the
    // window — `getSalesCount` is a SQL aggregate (no row materialisation,
    // no silent truncation). The previous path fetched up to 5000 rows
    // just to check `.isEmpty`.
    final salesCount = await _db.salesDao.getSalesCount(
      storeId,
      startDate: weekAgo,
      endDate: today,
    );
...
ما التقرير الذي تريد الاطلاع عليه؟''',
  ...
  Future<AssistantResponse> _handleRecommendationsQuery(String storeId) async {
    final lowStock = await _db.productsDao.getLowStockProducts(storeId);
    final todayTotal = await _db.salesDao.getTodayTotal(storeId, '');
    // Wave 8 (P0-33): SQL aggregate, no truncation hazard. Recommendations
    // gate on `totalDebt > 1000` — the bounded fold path could miss that
    // threshold for high-debtor-count stores and silently skip the tip.
    final totalDebt = await _db.accountsDao.getTotalReceivable(storeId);
