...
final aiFraudDetectionServiceProvider =
    Provider<AiFraudDetectionService>((ref) {
...
final fraudAlertsProvider =
    FutureProvider.autoDispose<List<FraudAlert>>((ref) async {
...
final behaviorScoresProvider =
    FutureProvider.autoDispose<List<BehaviorScore>>((ref) async {
  final service = ref.watch(aiFraudDetectionServiceProvider);
...
final fraudSummaryProvider =
    FutureProvider.autoDispose<FraudDetectionSummary>((ref) async {
  final service = ref.watch(aiFraudDetectionServiceProvider);
  return service.getSummary(ref.read(currentStoreIdProvider)!);
});

/// مزود فلتر خطورة التنبيهات - Severity Filter Provider
final fraudSeverityFilterProvider =
    StateProvider<FraudSeverity?>((ref) => null);
