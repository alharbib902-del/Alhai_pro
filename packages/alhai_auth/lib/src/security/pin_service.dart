...
/// عدد التكرارات لـ PBKDF2 على الأجهزة الأصلية (Android/iOS/Desktop).
///
/// 100,000 iterations provides strong brute-force resistance but may cause
/// noticeable latency (~200-500ms) on lower-end mobile devices. This is an
/// acceptable tradeoff for PIN security since verification is infrequent.
const int kPbkdf2IterationsNative = 100000;

/// عدد التكرارات لـ PBKDF2 على الويب.
///
/// Web (dart2js / dart2wasm) runs significantly slower for compute-heavy loops.
/// 100K iterations can cause 2-5 second UI freezes on the main thread.
/// 10K iterations still provides adequate brute-force resistance for a 4-6 digit
/// PIN (the search space is tiny anyway), while keeping latency under 200ms.
const int kPbkdf2IterationsWeb = 10000;

/// Platform-aware iteration count.
///
/// Uses [kPbkdf2IterationsWeb] (10K) on web to avoid main-thread jank,
/// and [kPbkdf2IterationsNative] (100K) on mobile/desktop for maximum security.
///
/// Future consideration: migrate to Argon2id for better resistance to
/// GPU/ASIC attacks while maintaining comparable performance. The existing
/// version migration mechanism (_currentVersion / _migratePin) supports this.
int get kPbkdf2Iterations =>
    kIsWeb ? kPbkdf2IterationsWeb : kPbkdf2IterationsNative;
