...
void main() {
  ...
  group('InvoiceChainService - chaining behavior', () {
    test('updateLastHash stores hash for retrieval by next invoice', () async {
      ...
      await service.updateLastHash(storeId: 'store-1', invoiceHash: firstHash);
    ...
    test('each invoice\'s PIH equals the previous invoice\'s hash', () async {
      ...
      await service.updateLastHash(storeId: storeId, invoiceHash: 'hash1');

      // Invoice #2 -- uses hash1
      final pih2 = await service.getPreviousHash(storeId: storeId);
      expect(pih2, 'hash1');
      await service.updateLastHash(storeId: storeId, invoiceHash: 'hash2');

      // Invoice #3 -- uses hash2
      final pih3 = await service.getPreviousHash(storeId: storeId);
      expect(pih3, 'hash2');
      await service.updateLastHash(storeId: storeId, invoiceHash: 'hash3');
    ...
    test('maintains separate chains per store', () async {
      await service.updateLastHash(storeId: 'store-A', invoiceHash: 'A-hash');
      await service.updateLastHash(storeId: 'store-B', invoiceHash: 'B-hash');
  ...
  group('InvoiceChainService - computeAndStore', () {
    ...
    test(
      'computeAndStore result matches direct InvoiceHasher output',
      () async {
        const storeId = 'store-1';
        const xml =
            '<Invoice><cbc:ID xmlns:cbc="urn:cbc">42</cbc:ID></Invoice>';
    ...
    test(
      'computeAndStore returns a valid base64 string (32-byte SHA-256)',
      () async {
        const xml =
  ...
  group('InvoiceChainService - resetChain', () {
    test(
      'resetChain clears chain so next getPreviousHash returns seed',
      () async {
        const storeId = 'store-1';
        ...
        expect(await service.getPreviousHash(storeId: storeId), 'some-hash');
    ...
    test('resetChain for one store does not affect another store', () async {
      ...
      expect(await service.getPreviousHash(storeId: 'store-B'), 'B-hash');
    });

    test(
      'resetChain is idempotent (safe to call with no prior state)',
      () async {
    ...
    test('can rebuild chain after reset', () async {
      ...
      expect(await service.getPreviousHash(storeId: storeId), 'new-1');
    });
  });
