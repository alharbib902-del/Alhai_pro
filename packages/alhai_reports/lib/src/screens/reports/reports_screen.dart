...
final _reportsTodayStatsProvider = FutureProvider.autoDispose<SalesStats?>((
  ref,
) async {
...
final _reportsWeekStatsProvider = FutureProvider.autoDispose<SalesStats?>((
  ref,
) async {
  final storeId = ref.watch(currentStoreIdProvider);
  if (storeId == null) return null;
...
final _reportsMonthStatsProvider = FutureProvider.autoDispose<SalesStats?>((
  ref,
) async {
  final storeId = ref.watch(currentStoreIdProvider);
  if (storeId == null) return null;
...
final _reportsPeriodPaymentStatsProvider = FutureProvider.autoDispose
    .family<List<PaymentMethodStats>, ({DateTime start, DateTime end})>((
      ...
      final db = GetIt.I<AppDatabase>();
      return db.salesDao.getPaymentMethodStats(
        storeId,
        startDate: range.start,
        endDate: range.end,
      );
...
class _ReportsScreenState extends ConsumerState<ReportsScreen> {
      ...
      child: Scaffold(
        ...
        body: Column(
          children: [
            ...
            // Reports Grid
            Expanded(child: _buildReportsGrid(isDesktop, l10n)),
          ],
        ),
      ),
    );
  ...
  Widget _buildReportsGrid(bool isDesktop, AppLocalizations l10n) {
    ...
    final paymentAsync = ref.watch(
      _reportsPeriodPaymentStatsProvider(dateRange),
    );
