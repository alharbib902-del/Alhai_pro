...
import 'whatsapp/wasender_api_client.dart';
...
class WhatsAppReceiptService {
  final WaSenderApiClient _apiClient; // ignore: unused_field
  ...
  static String formatReceipt({
    ...
    for (final item in items) {
      buffer.writeln(
        '${item.quantity}x ${item.name} ... ${item.total.toStringAsFixed(2)} $_currency',
      );
    }
    buffer.writeln('──────────');
    ...
    buffer.writeln(
      'الضريبة $taxPercent%: ${tax.toStringAsFixed(2)} $_currency',
    );
