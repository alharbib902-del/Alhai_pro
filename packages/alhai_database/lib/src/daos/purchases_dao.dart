...
class PurchasesDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<int> markAsSynced(String id) {
    return (update(purchasesTable)..where((p) => p.id.equals(id)))
        .write(PurchasesTableCompanion(syncedAt: Value(DateTime.now())));
  }
  ...
  Future<void> insertPurchaseItems(
      List<PurchaseItemsTableCompanion> items) async {
    await batch((b) {
  ...
  Future<int> deletePurchaseItems(String purchaseId) {
    return (delete(purchaseItemsTable)
          ..where((i) => i.purchaseId.equals(purchaseId)))
        .go();
  }
