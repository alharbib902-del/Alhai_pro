...
class OrgMembersDao extends DatabaseAccessor<AppDatabase>
  ...
  Future<List<OrgMembersTableData>> getOrgMembers(String orgId) {
    return (select(orgMembersTable)..where((m) => m.orgId.equals(orgId))).get();
  }
  ...
  Future<OrgMembersTableData?> getMemberByUserId(String orgId, String userId) {
    return (select(orgMembersTable)
          ..where((m) => m.orgId.equals(orgId) & m.userId.equals(userId)))
        .getSingleOrNull();
  }

  Future<int> upsertOrgMember(OrgMembersTableCompanion member) =>
      into(orgMembersTable).insert(member, mode: InsertMode.insertOrReplace);
  ...
  Future<int> upsertUserStore(UserStoresTableCompanion userStore) =>
      into(userStoresTable).insert(userStore, mode: InsertMode.insertOrReplace);
