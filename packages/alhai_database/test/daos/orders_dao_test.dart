...
void main() {
  ...
  OrdersTableCompanion _makeOrder({
    String id = 'order-1',
  ...
  OrderItemsTableCompanion _makeOrderItem({
    String id = 'oi-1',
  ...
  group('OrdersDao', () {
    ...
    test('getPendingOrders returns all non-completed orders', () async {
      await db.ordersDao.createOrder(
          _makeOrder(id: 'o1', orderNumber: 'ORD-1', status: 'created'));
      await db.ordersDao.createOrder(
          _makeOrder(id: 'o2', orderNumber: 'ORD-2', status: 'confirmed'));
      await db.ordersDao.createOrder(
          _makeOrder(id: 'o3', orderNumber: 'ORD-3', status: 'delivered'));
      await db.ordersDao.createOrder(
          _makeOrder(id: 'o4', orderNumber: 'ORD-4', status: 'cancelled'));
    ...
    test('addOrderItem and getOrderItems', () async {
      await db.ordersDao.createOrder(_makeOrder());
      await db.ordersDao.addOrderItem(_makeOrderItem());
      await db.ordersDao.addOrderItem(_makeOrderItem(
        id: 'oi-2',
        productName: 'كولا',
        productId: 'prod-2',
    ...
    test('addOrderItems batch inserts', () async {
      ...
      await db.ordersDao.addOrderItems([
        _makeOrderItem(id: 'oi-1'),
        _makeOrderItem(id: 'oi-2', productId: 'prod-2'),
      ]);
