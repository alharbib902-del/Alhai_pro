...
void main() {
  ...
  group('Low Stock Query Performance', () {
    ...
    test('getLowStockWithCategory with 1000 products completes under 200ms',
      ...
      expect(sw.elapsedMilliseconds, lessThan(200),
          reason: 'Low stock with category JOIN should complete under 200ms '
              '(actual: ${sw.elapsedMilliseconds}ms)');
    });
  });
  ...
  group('Inventory Movement History Performance', () {
    ...
    test('getMovementsByProduct with 500 total movements completes under 200ms',
      ...
      expect(movements, isNotEmpty, reason: 'Should find movements for prod_0');
      expect(sw.elapsedMilliseconds, lessThan(200),
          reason: 'Movement history query should complete under 200ms '
              '(actual: ${sw.elapsedMilliseconds}ms)');
    });
    ...
    test('getUnsyncedMovements with 500 movements completes under 200ms',
      ...
      expect(unsynced, hasLength(500),
          reason: 'All 500 movements should be unsynced');
      expect(sw.elapsedMilliseconds, lessThan(200),
          reason: 'Unsynced movements query should complete under 200ms '
              '(actual: ${sw.elapsedMilliseconds}ms)');
    });
  });
}
