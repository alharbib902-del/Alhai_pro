...
void main() {
  ...
  group('Product FTS Search Performance', () {
    ...
    test('search 1000 products by partial English name completes under 200ms', () async {
      final sw = Stopwatch()..start();
      final results = await db.productsDao.searchProducts('Product 5', 'store-1');
      sw.stop();

      expect(results, isNotEmpty, reason: 'Search should return results');
      expect(sw.elapsedMilliseconds, lessThan(200),
          reason: 'English name search across 1000 products should complete under 200ms '
              '(actual: ${sw.elapsedMilliseconds}ms)');
    });

    test('search by barcode pattern completes under 100ms', () async {
      final sw = Stopwatch()..start();
      final results = await db.productsDao.searchProducts('69000005', 'store-1');
      sw.stop();

      expect(results, isNotEmpty, reason: 'Barcode pattern search should return results');
  ...
  group('Barcode Lookup Performance', () {
    ...
    test('getProductByBarcode with 1000 products completes under 50ms', () async {
      // Lookup a barcode in the middle of the dataset
      final sw = Stopwatch()..start();
      final product = await db.productsDao.getProductByBarcode('6900000500', 'store-1');
      sw.stop();

      expect(product, isNotNull, reason: 'Barcode lookup should find the product');
    ...
    test('quickFindByBarcode with 1000 products completes under 50ms', () async {
      final sw = Stopwatch()..start();
      final product = await db.productsDao.quickFindByBarcode('6900000750', 'store-1');
      sw.stop();

      expect(product, isNotNull, reason: 'Quick barcode lookup should find the product');
    ...
    test('barcode lookup for non-existent barcode completes under 50ms', () async {
      final sw = Stopwatch()..start();
      final product = await db.productsDao.getProductByBarcode('9999999999', 'store-1');
      sw.stop();

      expect(product, isNull, reason: 'Non-existent barcode should return null');
      expect(sw.elapsedMilliseconds, lessThan(50),
  ...
  group('Products Count Query Performance', () {
    ...
    test('getProductById for 100 random lookups completes under 200ms', () async {
      final sw = Stopwatch()..start();
      for (int i = 0; i < 100; i++) {
    ...
    test('getProductsByCategory with 1000 products completes under 50ms', () async {
      // No products have categoryId set, so this tests the empty result path performance
      final sw = Stopwatch()..start();
      final products = await db.productsDao.getProductsByCategory('cat-1', 'store-1');
      sw.stop();
