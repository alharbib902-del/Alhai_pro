...
void main() {
  ...
  group('Sync Queue Lifecycle Flow', () {
    ...
    test(
      'retry logic: failed items increment attempts and remain pending',
      () async {
        // Enqueue an item
        await db.syncQueueDao.enqueue(
          id: 'retry-1',
    ...
    test('conflict resolution flow: conflict -> resolved', () async {
      ...
      await db.syncQueueDao.markAsConflict(
        'conflict-1',
        'Version mismatch: server has newer data',
      );
    ...
    test(
      'mixed operations: multiple items through different lifecycle paths',
      () async {
        // Enqueue 5 items
        for (int i = 1; i <= 5; i++) {
          await db.syncQueueDao.enqueue(
