  ...
  group('SyncService', () {
    group('enqueue', () {
      ...
      test('encodes payload as JSON', () async {
        ...
        final captured = verify(() => mockSyncQueueDao.enqueue(
              id: any(named: 'id'),
              tableName: any(named: 'tableName'),
              recordId: any(named: 'recordId'),
              operation: any(named: 'operation'),
    ...
    group('enqueueDelete', () {
      test('delegates to enqueue with delete operation and deleted payload',
          () async {
        when(() => mockSyncQueueDao.findByIdempotencyKey(any()))
            .thenAnswer((_) async => null);
        when(() => mockSyncQueueDao.enqueue(
    ...
    group('watchPendingItems', () {
      test('delegates to DAO stream', () {
        final items = [createSyncQueueItem()];
        when(() => mockSyncQueueDao.watchPendingItems())
    ...
    group('watchConflictItems', () {
      test('delegates to DAO stream', () {
        final items = [createSyncQueueItem(status: 'conflict')];
        when(() => mockSyncQueueDao.watchConflictItems())
            .thenAnswer((_) => Stream.value(items));
    ...
    group('watchConflictCount', () {
      test('delegates to DAO stream', () {
        when(() => mockSyncQueueDao.watchConflictCount())
    ...
    group('markResolved', () {
      test('delegates to DAO', () async {
        when(() => mockSyncQueueDao.markResolved(any()))
            .thenAnswer((_) async => 1);
    ...
    group('retryItem', () {
      test('delegates to DAO', () async {
        when(() => mockSyncQueueDao.retryItem(any()))
            .thenAnswer((_) async => 1);
    ...
    group('removeItem', () {
      test('delegates to DAO', () async {
        when(() => mockSyncQueueDao.removeItem(any()))
            .thenAnswer((_) async => 1);
    ...
    group('cleanup', () {
      test('delegates to DAO with default duration', () async {
        ...
        verify(() => mockSyncQueueDao.cleanupSyncedItems(
            olderThan: const Duration(days: 7))).called(1);
      });
      ...
      test('delegates to DAO with custom duration', () async {
        ...
        expect(result, 5);
        verify(() => mockSyncQueueDao.cleanupSyncedItems(
            olderThan: const Duration(days: 1))).called(1);
      });
    });
  });
}
