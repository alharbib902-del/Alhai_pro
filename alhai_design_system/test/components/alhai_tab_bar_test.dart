  ...
  group('AlhaiTabBar', () {
    ...
    testWidgets('shows subtitle when provided', (tester) async {
      await tester.pumpWidget(
        createTestWidget(
          AlhaiTabBar(
            tabs: const [AlhaiTabBarItem(label: 'Tab 1', subtitle: 'Details')],
            currentIndex: 0,
            onChanged: (_) {},
          ),
        ),
    ...
    test('AlhaiTabBarVariant has expected values', () {
      ...
      expect(
        AlhaiTabBarVariant.values,
        contains(AlhaiTabBarVariant.scrollable),
      );
    });

    test('AlhaiTabBarItem creates correctly', () {
      const item = AlhaiTabBarItem(label: 'Test', icon: Icons.star, badge: '5');
