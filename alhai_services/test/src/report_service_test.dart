...
class FakeReportsRepo implements ReportsRepository {
  ...
  Future<SalesSummary> getDailySummary(String storeId, DateTime date) async =>
      SalesSummary(
          ...
          profit: 2000.0);
  @override
  Future<List<SalesSummary>> getSalesSummaries(String storeId,
  ...
  @override
  Future<List<ProductSales>> getTopProducts(String storeId,
  ...
  @override
  Future<List<CategorySales>> getCategorySales(String storeId,
          {required DateTime startDate, required DateTime endDate}) async =>
      [
        const CategorySales(
            categoryId: 'c1',
  ...
  Future<Map<int, double>> getHourlySales(
      ...
      {8: 200.0, 14: 1200.0};
  @override
  Future<MonthlyComparison> getMonthlyComparison(String storeId,
...
void main() {
  ...
  group('ReportService', () {
    ...
    test('getDailySummary should return summary', () async {
      final s =
          await reportService.getDailySummary('store-1', DateTime(2026, 3, 15));
    ...
    test('getSalesSummaries should return for date range', () async {
      final summaries = await reportService.getSalesSummaries('store-1',
          startDate: DateTime(2026, 3, 1), endDate: DateTime(2026, 3, 7));
    ...
    test('getTopProducts should return list', () async {
      final products = await reportService.getTopProducts('store-1',
          startDate: DateTime(2026, 3, 1), endDate: DateTime(2026, 3, 31));
      expect(products, hasLength(1));
      expect(products.first.productName, equals('Coffee'));
    });

    test('getCategorySales should return list', () async {
      final categories = await reportService.getCategorySales('store-1',
          startDate: DateTime(2026, 3, 1), endDate: DateTime(2026, 3, 31));
    ...
    test('getHourlySales should return map', () async {
      final hourly =
          await reportService.getHourlySales('store-1', DateTime(2026, 3, 15));
      expect(hourly, isNotEmpty);
      expect(hourly[14], equals(1200.0));
    });

    test('getMonthlyComparison should return comparison', () async {
      final comparison = await reportService.getMonthlyComparison('store-1',
          year: 2026, month: 3);
