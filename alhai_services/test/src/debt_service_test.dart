...
class FakeDebtsRepository implements DebtsRepository {
  @override Future<Paginated<Debt>> getDebts(String storeId, {DebtType? type, bool? overdueOnly, int page = 1, int limit = 20}) async => Paginated(items: [], total: 0, page: page, limit: limit);
  ...
  @override Future<DebtSummary> getDebtSummary(String storeId) async => DebtSummary(totalCustomerDebts: 300.0, totalSupplierDebts: 700.0, overdueCount: 2, overdueAmount: 200.0);
}

void main() {
  late DebtService debtService;
  setUp(() { debtService = DebtService(FakeDebtsRepository()); });

  group('DebtService', () {
    test('should be created', () { expect(debtService, isNotNull); });
