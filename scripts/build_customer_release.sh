#!/usr/bin/env bash
# =============================================================================
# Alhai — customer_app release build
# =============================================================================
...
#   dist/customer_app-<versionName>-<buildNumber>.aab (copy for upload)
# =============================================================================
...
APP_DIR="$REPO_ROOT/customer_app"
...
echo -e "${GREEN}== customer_app release build ==${NC}"
echo ""

# --- Pre-flight ---------------------------------------------------------------

[ -d "$APP_DIR" ] || fail "customer_app directory missing at $APP_DIR"

[ -f "$APP_DIR/android/key.properties" ] \
    || fail "customer_app/android/key.properties missing — run generate_keystores.sh and fill it"

[ -f "$APP_DIR/android/app/google-services.json" ] \
    || note "customer_app/android/app/google-services.json missing — Firebase/FCM will be DISABLED at runtime. Build will still succeed."
...
mkdir -p "$DIST_DIR"
DIST_AAB="$DIST_DIR/customer_app-${VERSION_NAME}-${BUILD_NUMBER}.aab"
...
echo "  3. Keep a SECURE backup of customer_app/android/key.properties and the keystore .jks file."
