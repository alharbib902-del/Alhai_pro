  ...
  Future<void> _selectCustomer() async {
    ...
    if (customer != null && mounted) {
      setState(() {
        ...
        _creditLimitExceeded = (currentDebt + widget.total) > creditLimit;
      });
    }
  }
  ...
  void _addSplit() {
    ...
    if (amount <= 0) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('أدخل مبلغ صحيح'), backgroundColor: Colors.red),
      );
      return;
    }
    if (amount > 999999.99) {
  ...
  void _completePayment() {
    if (_selectedMethod == PaymentMethod.cash) {
      ...
      if (paid < widget.total) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('المبلغ المستلم أقل من الإجمالي'),
            backgroundColor: Colors.red,
          ),
        );
        return;
      ...
      if (_selectedCustomer == null) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('يجب اختيار العميل أولاً'),
            backgroundColor: Colors.red,
          ),
        );
        return;
      ...
      if (_creditLimitExceeded) {
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('تم تجاوز حد الدين للعميل'),
            backgroundColor: Colors.red,
          ),
        );
        return;
              ...
              children: [
                Row(
                  ...
                  children: [
                    ...
                    Text(
                      '${_selectedCustomer!.balance.toStringAsFixed(2)} ر.س',
                      style: TextStyle(
                        fontWeight: FontWeight.bold,
                        color: _selectedCustomer!.balance < 0 ? Colors.red : Colors.green,
                      ),
                    ),
                  ],
                ),
                ...
                children: [
                  ...
                  Text(
                    _splitRemaining <= 0.01
                        ? '✓'
                        : '${_splitRemaining.toStringAsFixed(2)} ر.س',
      ...
      child: InkWell(
        ...
        child: Container(
          ...
          decoration: BoxDecoration(
            borderRadius: BorderRadius.circular(12),
            border: Border.all(
              color: isSelected ? method.color : Colors.grey.withValues(alpha: 0.3),
