...
class ReturnsDataTable extends StatelessWidget {
        ...
        child: Center(
          child: Column(
            children: [
              ...
              Text(l10n.noReturns, style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: isDark ? Colors.white : AppColors.textPrimary)),
      ...
      child: ConstrainedBox(
        ...
        child: DataTable(
          headingRowColor: WidgetStateProperty.all(
            isDark ? const Color(0xFF0F172A).withValues(alpha: 0.5) : AppColors.grey50,
          ),
          dataRowColor: WidgetStateProperty.resolveWith((states) {
            if (states.contains(WidgetState.hovered)) {
              return isDark ? Colors.white.withValues(alpha: 0.03) : AppColors.grey50;
            }
          ...
          rows: returns.map((ret) => _buildRow(context, ret, isDark, l10n)).toList(),
        ),
      ),
    );
  }

  DataRow _buildRow(BuildContext context, ReturnModel ret, bool isDark, AppLocalizations l10n) {
    return DataRow(
      cells: [
        // Return number
        DataCell(
          Row(
            ...
            children: [
              ...
              InkWell(
                ...
                child: Icon(Icons.copy_outlined, size: 14, color: isDark ? AppColors.textMutedDark : AppColors.grey300),
              ),
            ],
          ),
        ),
        ...
        // Reason
        DataCell(_buildReasonBadge(ret.reason, l10n, isDark)),
        // Amount
        DataCell(Text(
        ...
        // Status
        DataCell(_buildStatusBadge(ret.status, l10n, isDark)),
        // Actions
        DataCell(Center(child: _buildActions(ret, l10n, isDark))),
      ],
    );
  }
  ...
  Widget _buildAvatar(ReturnModel ret, bool isDark) {
    ...
    return CircleAvatar(
      ...
      child: Text(initial, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: isDark ? AppColors.textMutedDark : AppColors.textSecondary)),
    );
  }
  ...
  Widget _buildReasonBadge(String reason, AppLocalizations l10n, bool isDark) {
    ...
    switch (reason) {
      ...
      case 'wrong':
        ...
        bgColor = isDark ? AppColors.grey500.withValues(alpha: 0.15) : AppColors.grey100;
        textColor = isDark ? AppColors.textSecondaryDark : AppColors.textSecondary;
        text = l10n.wrongProduct;
      case 'customer_request':
        icon = Icons.assignment_return_outlined;
        bgColor = isDark ? AppColors.info.withValues(alpha: 0.15) : const Color(0xFFDBEAFE);
  ...
  Widget _buildActions(ReturnModel ret, AppLocalizations l10n, bool isDark) {
    return Row(
      ...
      children: [
        // View
        _actionButton(Icons.visibility_outlined, () => onView(ret), isDark),
        ...
        if (ret.status == 'pending') ...[
          PopupMenuButton<String>(
            ...
            child: Container(
              ...
              child: Icon(Icons.more_horiz, size: 16, color: isDark ? AppColors.textMutedDark : AppColors.textSecondary),
            ),
          ),
        ] else ...[
          // Print for completed
          if (ret.status == 'refunded')
            _actionButton(Icons.print_outlined, () {}, isDark),
        ],
      ],
    );
  }
  ...
  Widget _actionButton(IconData icon, VoidCallback onTap, bool isDark) {
    return InkWell(
      ...
      child: Container(
        width: 30, height: 30,
        alignment: Alignment.center,
        child: Icon(icon, size: 16, color: isDark ? AppColors.textMutedDark : AppColors.textSecondary),
      ),
    );
  }
