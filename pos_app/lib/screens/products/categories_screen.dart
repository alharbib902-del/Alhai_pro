/// Categories Screen - شاشة إدارة التصنيفات
///
/// شاشة إدارة تصنيفات المنتجات مع:
/// - عرض Split View (يسار: شجرة التصنيفات، يمين: التفاصيل/التعديل)
/// - دعم dark/light mode
/// - دعم 7 لغات
...
// ============================================================================
// Provider — يجلب CategoriesTableData مباشرة من DAO (يحتوي color, icon, nameEn)
// ============================================================================

final _categoriesTableProvider =
    FutureProvider.autoDispose<List<CategoriesTableData>>((ref) async {
  final storeId = ref.watch(currentStoreIdProvider) ?? kDemoStoreId;
  final dao = getIt<AppDatabase>().categoriesDao;
  return dao.getAllCategories(storeId);
});

// ============================================================================
// الألوان والأيقونات المتاحة للتصنيفات
// ============================================================================
...
/// مساعد لتحويل لون hex من/إلى Color
...
/// مساعد لتحويل أيقونة من/إلى int
...
class _CategoriesScreenState extends ConsumerState<CategoriesScreen> {
  ...
  Future<void> _saveCategory() async {
    ...
    try {
      ...
      ref.invalidate(_categoriesTableProvider);
  ...
  Widget _buildSplitView(bool isDark, AppLocalizations l10n) {
    return Row(
      ...
      children: [
        // اللوحة اليسرى — قائمة التصنيفات (40%)
        Expanded(
          flex: 4,
          child: _buildLeftPanel(isDark, l10n),
        ),
        const SizedBox(width: 24),
        // اللوحة اليمنى — التفاصيل/التعديل (60%)
        Expanded(
          flex: 6,
  ...
  Widget _buildMobileView(bool isDark, AppLocalizations l10n) {
    ...
    return _buildLeftPanel(isDark, l10n);
  }

  // ============================================================================
  // اللوحة اليسرى — قائمة التصنيفات
  // ============================================================================

  Widget _buildLeftPanel(bool isDark, AppLocalizations l10n) {
    final categoriesAsync = ref.watch(_categoriesTableProvider);
          ...
          Expanded(
            child: categoriesAsync.when(
              data: (categories) {
                ...
                // فصل التصنيفات الرئيسية والفرعية
                ...
                return ListView.builder(
                  ...
                  itemBuilder: (context, index) {
                    ...
                    return _CategoryListItem(
                      category: category,
                      children: children.cast<CategoriesTableData>(),
                      isSelected: _selectedCategoryId == category.id,
                      isDark: isDark,
                      onTap: () => _selectCategory(category),
                      onChildTap: (child) => _selectCategory(child),
                      selectedChildId: _selectedCategoryId,
                    );
                  },
                );
              },
  ...
  // ============================================================================
  // اللوحة اليمنى — التفاصيل/التعديل
  // ============================================================================
        ...
        child: Form(
          ...
          child: Column(
            ...
            children: [
              ...
              // التصنيف الأب
              _buildParentCategoryField(isDark, l10n),
  ...
  Widget _buildParentCategoryField(bool isDark, AppLocalizations l10n) {
    final categoriesAsync = ref.watch(_categoriesTableProvider);

    return categoriesAsync.when(
      data: (categories) {
        // استبعاد التصنيف الحالي من القائمة
...
class _CategoryListItem extends StatefulWidget {
  ...
  final List<CategoriesTableData> children;
  final bool isSelected;
  final bool isDark;
  final VoidCallback onTap;
  final void Function(CategoriesTableData) onChildTap;
...
class _CategoryListItemState extends State<_CategoryListItem> {
  ...
  Widget build(BuildContext context) {
    ...
    final icon = _parseIcon(widget.category.icon);
          ...
          Padding(
            ...
            child: Column(
              children: widget.children.map((child) {
                ...
                final childIcon = _parseIcon(child.icon);
                final isChildSelected =
                    widget.selectedChildId == child.id;

                return _ChildCategoryTile(
                  category: child,
                  color: childColor,
                  icon: childIcon,
...
class _ChildCategoryTile extends StatefulWidget {
  final CategoriesTableData category;
  final Color color;
  final IconData icon;
  final bool isSelected;
  ...
  @override
  State<_ChildCategoryTile> createState() => _ChildCategoryTileState();
}

class _ChildCategoryTileState extends State<_ChildCategoryTile> {
  bool _isHovered = false;
