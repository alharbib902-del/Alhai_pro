...
///
/// يوفر الوصول للتصنيفات من قاعدة البيانات المحلية (Drift)
...
/// تنفيذ محلي لـ CategoriesRepository
class LocalCategoriesRepository implements CategoriesRepository {
  ...
  Future<List<Category>> getChildCategories(String parentId) async {
    // نحتاج إلى storeId، نستخدم الأب للحصول عليه
    final parent = await _db.categoriesDao.getCategoryById(parentId);
    if (parent == null) {
      return [];
    }
    final results = await _db.categoriesDao.getSubCategories(parentId, parent.storeId);
    return results.map(_toCategory).toList();
  }
