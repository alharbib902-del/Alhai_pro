...
class LoyaltyDao extends DatabaseAccessor<AppDatabase> with _$LoyaltyDaoMixin {
  ...
  Future<void> addPoints(String customerId, String storeId, int points) async {
    ...
    if (loyalty != null) {
      await (update(loyaltyPointsTable)
        ..where((l) => l.id.equals(loyalty.id)))
        .write(LoyaltyPointsTableCompanion(
          currentPoints: Value(loyalty.currentPoints + points),
          totalEarned: Value(loyalty.totalEarned + points),
          tierLevel: Value(_calculateTier(loyalty.totalEarned + points)),
  ...
  Future<bool> redeemPoints(String customerId, String storeId, int points) async {
    final loyalty = await getCustomerLoyalty(customerId, storeId);
    if (loyalty == null || loyalty.currentPoints < points) {
      return false;
    }
    ...
    await (update(loyaltyPointsTable)
      ...
      .write(LoyaltyPointsTableCompanion(
        currentPoints: Value(loyalty.currentPoints - points),
        totalRedeemed: Value(loyalty.totalRedeemed + points),
        updatedAt: Value(DateTime.now()),
      ));
