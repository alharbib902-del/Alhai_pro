...
class PurchasesDao extends DatabaseAccessor<AppDatabase> with _$PurchasesDaoMixin {
  ...
  Future<int> insertPurchase(PurchasesTableCompanion purchase) => into(purchasesTable).insert(purchase);
  Future<bool> updatePurchase(PurchasesTableData purchase) => update(purchasesTable).replace(purchase);

  Future<int> updateStatus(String id, String status) {
    return (update(purchasesTable)..where((p) => p.id.equals(id))).write(PurchasesTableCompanion(status: Value(status), updatedAt: Value(DateTime.now())));
  }

  Future<int> receivePurchase(String id) {
    return (update(purchasesTable)..where((p) => p.id.equals(id))).write(PurchasesTableCompanion(status: const Value('received'), receivedAt: Value(DateTime.now()), updatedAt: Value(DateTime.now())));
  }

  Future<int> deletePurchase(String id) => (delete(purchasesTable)..where((p) => p.id.equals(id))).go();

  Future<int> markAsSynced(String id) {
    return (update(purchasesTable)..where((p) => p.id.equals(id))).write(PurchasesTableCompanion(syncedAt: Value(DateTime.now())));
  }

  // Purchase items
  Future<List<PurchaseItemsTableData>> getPurchaseItems(String purchaseId) {
    return (select(purchaseItemsTable)..where((i) => i.purchaseId.equals(purchaseId))).get();
  }

  Future<void> insertPurchaseItems(List<PurchaseItemsTableCompanion> items) async {
    await batch((b) { b.insertAll(purchaseItemsTable, items); });
  }
