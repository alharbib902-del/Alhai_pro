...
class _CreateInvoiceScreenState extends ConsumerState<CreateInvoiceScreen> {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(AppLocalizations.of(context).productSearchFailed('$e')),
  ...
  Future<void> _searchCustomers(String query) async {
    ...
    try {
      ...
      if (storeId == null) return;
      final results =
          await _db.customersDao.searchCustomers(query, storeId);
      if (mounted) {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(
            content: Text(AppLocalizations.of(context).customerSearchFailed('$e')),
            backgroundColor: AppColors.error,
          ),
        );
      }
  ...
  void _addProduct(ProductsTableData product) {
    ...
    setState(() {
      if (existing >= 0) {
        _items[existing] =
            _items[existing].copyWithQty(_items[existing].qty + 1);
      } else {
  ...
  Widget _buildSelectedCustomerChip(ColorScheme colorScheme, AppLocalizations l10n) {
      ...
      child: Column(
        ...
        children: [
          ...
          _buildTotalRow(l10n.subtotal,
              '${_subtotal.toStringAsFixed(2)} ${l10n.sar}', colorScheme),
          const SizedBox(height: AlhaiSpacing.sm),
          // Discount input
          Row(
            children: [
