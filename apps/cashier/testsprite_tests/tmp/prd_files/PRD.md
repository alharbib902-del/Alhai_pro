...
- **Login Method:** OTP via phone number verification
- **Flow:** Phone input > Click "التالي" (Next) > Enter OTP > Store selection > POS screen
- **Test Credentials:**
  - **Phone Number:** `0500000001` (enter in the phone input field)
  - **Country Code:** `+966` (Saudi Arabia, pre-selected by default)
  - **OTP Code:** `123456` (test OTP for development)
...
### Login Steps (for testing):
1. Navigate to `/#/login`
2. Enter phone number `0500000001` in the phone input field
3. Click "التالي" (Next) button
4. Enter OTP code `123456` in the verification field
5. Click verify/confirm button
6. Select store "سوبرماركت الحي" if store selection appears
7. App redirects to POS screen (`/#/pos`)
