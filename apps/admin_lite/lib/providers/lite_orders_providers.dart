    ...
    FutureProvider.autoDispose<List<OrderWithCustomer>>((ref) async {
      ...
      try {
        final results = await Future.wait([
          db.ordersDao.getOrdersWithCustomer(
            storeId,
            status: 'confirmed',
            limit: 50,
          ),
        ...
        sentry.reportError(e, stackTrace: st, hint: 'liteActiveOrdersProvider');
        return [];
      }
    });
...
final liteOrdersByStatusProvider = FutureProvider.autoDispose
    .family<List<OrderWithCustomer>, String?>((ref, status) async {
      ...
      final db = GetIt.I<AppDatabase>();
      return db.ordersDao.getOrdersWithCustomer(
        storeId,
        status: status,
        limit: 100,
      );
    ...
    FutureProvider.autoDispose<List<OrderWithCustomer>>((ref) async {
      ...
      try {
        ...
        sentry.reportError(
          e,
          stackTrace: st,
          hint: 'liteDeliveryOrdersProvider',
        );
        return [];
      }
    });
...
final liteOrderHistoryProvider =
    FutureProvider.autoDispose<List<OrderWithCustomer>>((ref) async {
      ...
      try {
        ...
        sentry.reportError(e, stackTrace: st, hint: 'liteOrderHistoryProvider');
        return [];
      }
    });
