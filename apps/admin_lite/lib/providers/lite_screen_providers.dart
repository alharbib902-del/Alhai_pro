/// Lite Screen Providers
///
/// Riverpod providers for all Admin Lite screens. Each provider
/// queries the local Drift database via DAOs (GetIt-injected AppDatabase).
/// Follows the same pattern as `lite_dashboard_providers.dart`.
library;
...
final liteActiveOrdersProvider = FutureProvider.autoDispose<List<OrderWithCustomer>>((ref) async {
  final storeId = ref.watch(currentStoreIdProvider);
  if (storeId == null) return [];
  ...
  try {
    final results = await Future.wait([
      db.ordersDao.getOrdersWithCustomer(storeId, status: 'confirmed', limit: 50),
      db.ordersDao.getOrdersWithCustomer(storeId, status: 'preparing', limit: 50),
      db.ordersDao.getOrdersWithCustomer(storeId, status: 'ready', limit: 50),
      db.ordersDao.getOrdersWithCustomer(storeId, status: 'out_for_delivery', limit: 50),
    ]);
...
final liteOrderDetailProvider = FutureProvider.autoDispose.family<OrderWithItems?, String>((ref, orderId) async {
  final db = GetIt.I<AppDatabase>();
  return db.ordersDao.getOrderWithItems(orderId);
});

/// Provider: Orders filtered by a specific status
final liteOrdersByStatusProvider = FutureProvider.autoDispose.family<List<OrderWithCustomer>, String?>((ref, status) async {
  final storeId = ref.watch(currentStoreIdProvider);
  if (storeId == null) return [];

  final db = GetIt.I<AppDatabase>();
  return db.ordersDao.getOrdersWithCustomer(storeId, status: status, limit: 100);
});

/// Provider: Delivery orders (out_for_delivery)
final liteDeliveryOrdersProvider = FutureProvider.autoDispose<List<OrderWithCustomer>>((ref) async {
  final storeId = ref.watch(currentStoreIdProvider);
  if (storeId == null) return [];
  ...
  try {
    final results = await Future.wait([
      db.ordersDao.getOrdersWithCustomer(storeId, status: 'out_for_delivery', limit: 50),
      db.ordersDao.getOrdersWithCustomer(storeId, status: 'delivered', limit: 20),
    ]);
    final all = <OrderWithCustomer>[];
    for (final list in results) {
      all.addAll(list as List<OrderWithCustomer>);
...
final liteOrderHistoryProvider = FutureProvider.autoDispose<List<OrderWithCustomer>>((ref) async {
  final storeId = ref.watch(currentStoreIdProvider);
  if (storeId == null) return [];
  ...
  try {
    final results = await Future.wait([
      db.ordersDao.getOrdersWithCustomer(storeId, status: 'delivered', limit: 50),
      db.ordersDao.getOrdersWithCustomer(storeId, status: 'cancelled', limit: 50),
    ]);
    final all = <OrderWithCustomer>[];
    for (final list in results) {
      all.addAll(list as List<OrderWithCustomer>);
