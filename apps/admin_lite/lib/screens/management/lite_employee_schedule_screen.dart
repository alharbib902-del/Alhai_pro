            ...
            child: Column(
              ...
              children: [
                ...
                if (todayShifts.isNotEmpty) ...[
                  ...
                  ...todayShifts.map((s) => _buildScheduleCard(context, s, isDark, true)),
                  const SizedBox(height: AlhaiSpacing.lg),
                ],
                if (otherShifts.isNotEmpty) ...[
                  _buildSectionTitle(l10n.next, Icons.schedule, isDark),
                  const SizedBox(height: AlhaiSpacing.sm),
                  ...otherShifts.map((s) => _buildScheduleCard(context, s, isDark, false)),
                ],
                const SizedBox(height: AlhaiSpacing.lg),
              ],
            ),
        ...
        children: days.asMap().entries.map((entry) {
          ...
          return Expanded(
            child: Container(
              ...
              child: Column(
                children: [
                  Text(entry.value, style: TextStyle(fontSize: 11, fontWeight: isToday ? FontWeight.bold : FontWeight.normal, color: isToday ? Colors.white : (isDark ? Colors.white54 : Colors.black54))),
                  const SizedBox(height: AlhaiSpacing.xxxs),
                  Text('${dayDate.day}', style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: isToday ? Colors.white : (isDark ? Colors.white : Colors.black87))),
                ],
              ),
            ),
          );
  ...
  Widget _buildSectionTitle(String title, IconData icon, bool isDark) {
    return Row(
      children: [
        ...
        Text(title, style: TextStyle(fontSize: 16, fontWeight: FontWeight.bold, color: isDark ? Colors.white : null)),
      ],
    );
  }
      ...
      child: Row(
        children: [
          Container(
            ...
            child: Column(
              children: [
                Text(startTime, style: TextStyle(fontSize: 12, fontWeight: FontWeight.bold, color: color)),
                Text(endTime, style: TextStyle(fontSize: 11, color: color.withValues(alpha: 0.7))),
              ],
            ),
          ),
          ...
          CircleAvatar(
            ...
            child: Text(name.isNotEmpty ? name.substring(0, 1) : '?', style: const TextStyle(fontWeight: FontWeight.bold, color: AlhaiColors.primary, fontSize: 14)),
          ),
          const SizedBox(width: AlhaiSpacing.sm),
          Expanded(
            child: Column(
              crossAxisAlignment: CrossAxisAlignment.start,
              children: [
                Text(name, style: TextStyle(fontSize: 14, fontWeight: FontWeight.w600, color: isDark ? Colors.white : Colors.black87)),
                Text(shift.status, style: TextStyle(fontSize: 12, color: isDark ? Colors.white38 : Theme.of(context).colorScheme.onSurfaceVariant)),
              ],
            ),
          ),
          Container(
            ...
            decoration: BoxDecoration(
              color: isOpen ? AlhaiColors.success.withValues(alpha: 0.12) : (isDark ? Colors.white.withValues(alpha: 0.06) : Colors.grey.shade100),
