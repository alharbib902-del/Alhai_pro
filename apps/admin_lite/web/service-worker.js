/**
 * Alhai Admin Lite - PWA Service Worker
 * Cache strategy:
      ...
      await Promise.allSettled(
        PRECACHE_ASSETS.map((url) =>
          cache.add(url).catch((err) =>
            console.warn(`[SW:admin-lite] Precache failed for ${url}:`, err)
          )
        )
      );
      console.log('[SW:admin-lite] Installed – precache complete.');
...
self.addEventListener('activate', (event) => {
  event.waitUntil(
    (async () => {
      ...
      console.log('[SW:admin-lite] Activated – stale caches removed:', staleCaches);
