...
class _ProductFormScreenState extends ConsumerState<ProductFormScreen> {
  ...
  Future<void> _saveProduct() async {
    ...
    try {
      ...
      // M1: auto-generate an EAN-13 barcode when a new product is being
      // created without one. Only applies on the create path — editing to
      // blank is a deliberate "clear the barcode" action and must be
      // respected. Retries on the astronomically-unlikely store-level
      // collision; falls through with an empty barcode if all attempts
      // collide (rather than blocking the save).
      var effectiveBarcode = barcode;
      var autoGenerated = false;
      if (!widget.isEditing && effectiveBarcode.isEmpty) {
        for (var attempt = 0; attempt < 5; attempt++) {
          final candidate = BarcodeValidator.generateEan13();
          final existing = await db.productsDao.getProductByBarcode(
            candidate,
            storeId,
          );
          if (existing == null) {
            effectiveBarcode = candidate;
            autoGenerated = true;
            break;
          }
        }
      ...
      if (effectiveBarcode.isNotEmpty) {
      ...
      if (widget.isEditing) {
        ...
        final companion = ProductsTableCompanion(
          ...
          barcode: drift.Value(effectiveBarcode.isEmpty ? null : effectiveBarcode),
        ...
        try {
          ...
          await db.auditLogDao.log(
            ...
            newValue: {
              'name': name,
              'barcode': effectiveBarcode.isEmpty ? null : effectiveBarcode,
              ...
              if (autoGenerated) 'barcode_auto_generated': true,
            },
        ...
        if (mounted) {
          // M1: surface the auto-generated barcode so the user can print /
          // stick a label to the physical stock. Without this, the generated
          // code is invisible until they reopen the product.
          final successMsg = autoGenerated
              ? '${l10n.productAddedSuccess} (${l10n.barcode}: $effectiveBarcode)'
              : l10n.productAddedSuccess;
          ScaffoldMessenger.of(context).showSnackBar(
            SnackBar(
              content: Text(successMsg),
              backgroundColor: AppColors.success,
              duration: autoGenerated
