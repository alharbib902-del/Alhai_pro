...
class _SecuritySettingsScreenState
  ...
  Future<void> _saveSecuritySetting(String key, String value) async {
    ...
    final db = getIt<AppDatabase>();
    await saveSettingWithSync(db: db, storeId: storeId, key: key, value: value, ref: ref);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Column(children: [
      AppHeader(title: l10n.security, onMenuTap: isWideScreen ? null : () => Scaffold.of(context).openDrawer(),
          onNotificationsTap: () => context.push('/notifications'), notificationsCount: 3, userName: userName, userRole: userRole),
      Expanded(
        child: _isLoading
  ...
  Widget _buildContent(bool isWideScreen, bool isMediumScreen, bool isDark, AppLocalizations l10n) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      _buildSettingsGroup(l10n.pinSection, [
        _buildSettingsTile(icon: Icons.pin_rounded, title: l10n.createPinDesc,
            subtitle: _pinEnabled ? l10n.enabled : l10n.disabled,
            trailing: _pinEnabled
                ? const Icon(Icons.check_circle_rounded, color: AppColors.success)
                : Icon(Icons.cancel_rounded, color: isDark ? Colors.white.withValues(alpha: 0.3) : AppColors.textTertiary),
            onTap: _showPinOptions, isDark: isDark),
      ], isDark),
      ...
      _buildSettingsGroup(l10n.biometricSection, [
        if (_biometricAvailable)
          SwitchListTile(
            ...
            subtitle: Text(_biometricEnabled ? l10n.fingerprintDesc : l10n.fingerprintDesc,
                style: TextStyle(color: Theme.of(context).colorScheme.onSurfaceVariant, fontSize: 12)),
            value: _biometricEnabled, onChanged: _toggleBiometric)
        else
          _buildSettingsTile(icon: Icons.fingerprint_rounded, title: l10n.fingerprintOption, subtitle: l10n.fingerprintDesc,
              trailing: const Icon(Icons.info_outline_rounded, color: AppColors.warning), isDark: isDark),
      ], isDark),

      _buildSettingsGroup(l10n.sessionSection, [
        _buildSettingsTile(icon: Icons.access_time_rounded, title: l10n.autoLockOption,
            subtitle: _autoLockMinutes != null
                ? l10n.afterMinutes(int.tryParse(_autoLockMinutes!) ?? 30)
                : (_sessionRemaining != null ? l10n.afterMinutes(_sessionRemaining!.inMinutes) : l10n.disabled),
            trailing: IconButton(icon: Icon(Icons.refresh_rounded, color: Theme.of(context).colorScheme.onSurfaceVariant),
                onPressed: _loadSecuritySettings), isDark: isDark),
      ], isDark),

      _buildSettingsGroup(l10n.dangerZone, [
        _buildSettingsTile(icon: Icons.logout_rounded, title: l10n.logoutAllDevices, subtitle: l10n.logoutAllDevicesDesc, isDark: isDark, onTap: _showLogoutConfirmation, iconColor: AppColors.error),
        _buildSettingsTile(icon: Icons.delete_forever_rounded, title: l10n.clearAllData, subtitle: l10n.clearAllDataDesc, isDark: isDark, onTap: _showClearAllConfirmation, iconColor: AppColors.error),
      ], isDark),
    ]);
  }
  ...
  Widget _buildSettingsTile({required IconData icon, required String title, String? subtitle, Widget? trailing, VoidCallback? onTap, required bool isDark, Color? iconColor}) {
    return ListTile(
      ...
      trailing: trailing ?? AdaptiveIcon(Icons.chevron_left_rounded, color: isDark ? Colors.white.withValues(alpha: 0.3) : AppColors.textTertiary),
      onTap: onTap,
    );
  }
  ...
  void _showPinOptions() {
    ...
    showModalBottomSheet(context: context, builder: (context) => SafeArea(child: Column(mainAxisSize: MainAxisSize.min, children: [
      if (_pinEnabled) ...[
        ...
        ListTile(leading: const Icon(Icons.delete, color: Colors.red), title: Text(l10n.removePinOption), onTap: () { Navigator.pop(context); _showRemovePinConfirmation(); }),
      ] else ...[
        ListTile(leading: const Icon(Icons.add), title: Text(l10n.createPinOption), onTap: () { Navigator.pop(context); _showCreatePinDialog(); }),
      ],
    ])));
  }
    ...
    await showDialog(context: context, builder: (context) => StatefulBuilder(
      builder: (context, setDialogState) => AlertDialog(title: Text(l10n.createPinTitle), content: Column(mainAxisSize: MainAxisSize.min, children: [
        ...
        FilledButton(onPressed: () async {
          if (newPin == null || newPin!.length < 4) { setDialogState(() => error = l10n.enterNewPin); return; }
          if (newPin != confirmPin) { setDialogState(() => error = l10n.removePinConfirm); return; }
          final result = await PinService.createPin(newPin!);
          if (result.isSuccess) {
            if (context.mounted) Navigator.pop(context);
            await _saveSecuritySetting('security_pin_enabled', 'true');
            _loadSecuritySettings();
            if (mounted) ScaffoldMessenger.of(this.context).showSnackBar(SnackBar(content: Text(l10n.pinCreated)));
  ...
  Future<void> _showRemovePinConfirmation() async {
    ...
    final confirmed = await showDialog<bool>(context: context, builder: (context) => AlertDialog(
      ...
      actions: [
        ...
        FilledButton(onPressed: () => Navigator.pop(context, true), style: FilledButton.styleFrom(backgroundColor: Theme.of(context).colorScheme.error), child: Text(l10n.removeAction)),
      ],
    ));
    if (confirmed == true) {
      ...
      _loadSecuritySettings();
      if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(l10n.pinRemovedSuccess)));
    }
  }
  ...
  Future<void> _toggleBiometric(bool enable) async {
    ...
    if (enable) {
      ...
      if (success) {
        ...
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(l10n.fingerprintDesc)));
      } else {
        if (mounted) ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text(l10n.fingerprintDesc)));
      }
    } else {
  ...
  Future<void> _showLogoutConfirmation() async {
    ...
    final confirmed = await showDialog<bool>(context: context, builder: (context) => AlertDialog(
      ...
      actions: [
        TextButton(onPressed: () => Navigator.pop(context, false), child: Text(l10n.cancel)),
        FilledButton(onPressed: () => Navigator.pop(context, true), child: Text(l10n.logoutAllAction)),
      ],
    ));
    if (confirmed == true) {
  ...
  Future<void> _showClearAllConfirmation() async {
    ...
    final confirmed = await showDialog<bool>(context: context, builder: (context) => AlertDialog(
      ...
      actions: [
        TextButton(onPressed: () => Navigator.pop(context, false), child: Text(l10n.cancel)),
        FilledButton(onPressed: () => Navigator.pop(context, true), style: FilledButton.styleFrom(backgroundColor: Theme.of(context).colorScheme.error), child: Text(l10n.clearDataAction)),
      ],
    ));
    if (confirmed == true) {
