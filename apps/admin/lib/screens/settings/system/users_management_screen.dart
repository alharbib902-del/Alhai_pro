...
class _UsersManagementScreenState extends ConsumerState<UsersManagementScreen> {
            ...
            : SingleChildScrollView(
                ...
                child: Center(
                    child: ConstrainedBox(
                        ...
                        child: _buildContent(
                            isWideScreen, size.width >= 600, isDark, l10n)))),
      ),
    ]);
  }
  ...
  Widget _buildContent(bool isWideScreen, bool isMediumScreen, bool isDark,
    ...
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      _buildSettingsGroup('${l10n.users} (${_users.length})',
          _users.map((user) => _buildUserTile(user, isDark)).toList(), isDark),
    ]);
  }
      ...
      title: Row(children: [
        ...
        if (!user.active)
          Container(
              ...
              child: Text(l10n.disabledStatus,
          ...
          return [
            ...
            PopupMenuItem(
                ...
                child: Text(user.active
                    ? l10n.disableMenuAction
                    : l10n.enableMenuAction)),
            if (user.role != 'owner')
              PopupMenuItem(
                  value: 'delete',
      ...
      case 'disable':
        ...
        try {
          ...
          if (dbUser != null) {
            ...
            await syncService
                .enqueueUpdate(tableName: 'users', recordId: user.id, changes: {
              'id': user.id,
  ...
  void _logAuditEvent(
      AppDatabase db, String action, String entityId, String entityName) {
    try {
      final storeId = ref.read(currentStoreIdProvider) ?? '';
              ...
              builder: (context, setDialogState) => AlertDialog(
                      ...
                      actions: [
                        ...
                        FilledButton(
                            onPressed: () async {
                              final finalRole =
                                  user.role == 'owner' ? 'owner' : role;
                              try {
                                final db = getIt<AppDatabase>();
                                final dbUser =
                                    await db.usersDao.getUserById(user.id);
                                if (dbUser != null) {
                                  final updatedUser = UsersTableData(
                                      id: dbUser.id,
                                      orgId: dbUser.orgId,
