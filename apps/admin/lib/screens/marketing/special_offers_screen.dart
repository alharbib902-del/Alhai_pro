          ...
          child: promotionsAsync.when(
            ...
            data: (promotions) => SingleChildScrollView(
              ...
              child: _OffersContent(
                promotions: promotions, isWideScreen: isWideScreen,
                isMediumScreen: isMediumScreen, isDark: isDark, l10n: l10n,
              ),
            ),
          ),
        ),
...
class _OffersContent extends ConsumerWidget {
  ...
  const _OffersContent({
    required this.promotions, required this.isWideScreen,
  ...
  Future<void> _toggleActive(WidgetRef ref, PromotionsTableData promotion, bool value) async {
    try {
      final updated = PromotionsTableData(
        id: promotion.id, storeId: promotion.storeId, orgId: promotion.orgId,
        name: promotion.name, nameEn: promotion.nameEn,
        description: promotion.description, type: promotion.type,
        rules: promotion.rules, startDate: promotion.startDate,
        endDate: promotion.endDate, isActive: value,
        createdAt: promotion.createdAt, updatedAt: DateTime.now(),
            ...
            FilledButton(
              onPressed: () async {
                if (nameController.text.isNotEmpty) {
                  try {
                    await addPromotion(ref, name: nameController.text, type: type);
                  } catch (e) {
