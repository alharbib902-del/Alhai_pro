...
> For the full detailed reference, see [docs/ARCHITECTURE.md](docs/ARCHITECTURE.md).
...
Apps under `apps/` reference packages with `../../` prefixes. Root-level apps use `../` prefixes. Melos resolves all paths automatically via the `packages` glob in `melos.yaml`.
...
For critical tables (orders, deliveries), Supabase Realtime subscriptions push changes instantly to connected clients via `realtime_listener.dart`.
...
## 5. Database Overview / قاعدة البيانات
...
See [SECURITY.md](SECURITY.md) for the full security report.
...
ARB files live in `packages/alhai_l10n/lib/l10n/`. All user-facing strings use `AppLocalizations.of(context)`.
